################################################################################
#
#   YvO Differential Expression of Proteins — DEqMS Pipeline (Imputed)
#
...
#   Proteins: 2124 (log2-transformed, BPCA-imputed — no NAs)
...
cat("=== YvO DEqMS Pipeline (Imputed) ===\n\n")
cat(">> 0 — Setup\n")
...
REPORT_DIR <- file.path(base_dir, "03_DEP", "Imputed", "b_DEqMS", "b_reports")
DATA_DIR   <- file.path(base_dir, "03_DEP", "Imputed", "b_DEqMS", "c_data")
...
cat(sprintf("   Missing values: %d (%.1f%%) — imputed data, expect 0\n", n_missing, pct_missing))
...
stopifnot(all(pep_count >= 1, na.rm = TRUE))
...
for (i in seq_along(contrast_names)) {
  ...
  # Pi-value (raw p): Π = sca.P.Value ^ |logFC|
  res$pi_score <- res$sca.P.Value ^ abs(res$logFC)

  # Direction indicator: 1 = sig up, -1 = sig down, 0 = NS
  res$sig_pi <- case_when(
    res$pi_score < 0.05 & res$logFC > 0 ~  1L,
    res$pi_score < 0.05 & res$logFC < 0 ~ -1L,
    TRUE ~ 0L
  )
  res$sig_nominal_01 <- case_when(
    res$sca.P.Value < 0.01 & res$logFC > 0 ~  1L,
    res$sca.P.Value < 0.01 & res$logFC < 0 ~ -1L,
    TRUE ~ 0L
  )
...
da_summary <- tibble(
  ...
  n_sig_pi     = sapply(all_results, function(r) sum(r$pi_score < 0.05, na.rm = TRUE)),
  n_up_pi      = sapply(all_results, function(r) sum(r$pi_score < 0.05 & r$logFC > 0, na.rm = TRUE)),
  n_down_pi    = sapply(all_results, function(r) sum(r$pi_score < 0.05 & r$logFC < 0, na.rm = TRUE)),
  n_sig_nominal_01  = sapply(all_results, function(r) sum(r$sca.P.Value < 0.01, na.rm = TRUE)),
  n_up_nominal_01   = sapply(all_results, function(r) sum(r$sca.P.Value < 0.01 & r$logFC > 0, na.rm = TRUE)),
  n_down_nominal_01 = sapply(all_results, function(r) sum(r$sca.P.Value < 0.01 & r$logFC < 0, na.rm = TRUE))
)
...
cat("\n=== YvO DEqMS Pipeline (Imputed) Complete ===\n")
