# ── F5 prepare_data.R: WGCNA loading + shared eigengene/kME computation ───────
# Run BEFORE panel scripts. Saves intermediates to c_data/.
# - Loads WGCNA outputs (from c_data/wgcna/)
# - Loads imputed data + metadata
# - Computes module eigengenes (MEs) and signed kME
# - Saves all shared objects for panel consumption

# STAT AUDIT: 62 samples, ~2100 proteins, n non-grey modules (determined at
# runtime). corPvalueStudent() uses n=62 for ALL traits — p-values are LIBERAL
# for phenotype traits with missing data.
