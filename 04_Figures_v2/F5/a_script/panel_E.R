################################################################################
#   Figure 5 — Panel E: Functionally Annotated Hub Protein Networks
#   Self-contained: loads all data from c_data/ (saved by prepare_data.R)
#   6 key modules in a 2x3 grid, with GO-based hull annotations
#   + trait correlation sidebar heatmaps per module
#   Generates: panel_E_hub_network.pdf/png, c_data/05_panel_E_*.csv
################################################################################
...
trait_cor_df  <- read_csv(file.path(DAT, "wgcna/wgcna_module_trait_correlations.csv"),
                          show_col_types = FALSE)
trait_pval_df <- read_csv(file.path(DAT, "wgcna/wgcna_module_trait_pvalues_bh.csv"),
                          show_col_types = FALSE)
go_df_full    <- read_csv(file.path(DAT, "wgcna/wgcna_module_GO_enrichment.csv"),
                          show_col_types = FALSE)

message("Panel E: hub protein networks...")

PE_W <- 450  # panel width mm
PE_H <- 315  # panel height mm

# --- Text sizes ---
txt_title  <- scale_text(BASE_STAT, PE_W)
txt_sub    <- scale_text(BASE_GENE, PE_W)
txt_label  <- scale_text(BASE_GENE, PE_W)
txt_side   <- scale_text(BASE_GENE, PE_W) * 0.7
...
PERIPH_R_INNER_MULT <- 1.30
PERIPH_R_OUTER_MULT <- 2.05
PERIPH_ALPHA        <- 0.38
PERIPH_STROKE       <- 0.15
...
TRAIT_LABELS <- c(
  age_num = "Age", time_num = "Training", interaction = "Age\u00d7Train",
  VL_thick_cm = "VL Thick.", DXA_LBM_kg = "LBM", BMI = "BMI",
  Type_I_fCSA = "Type I", Type_II_fCSA = "Type II", deadlift_1rm_kg = "DL 1RM"
)
...
make_trait_sidebar <- function(mod) {
  ...
  ggplot(sidebar_df, aes(x = 1, y = trait_label, fill = r)) +
    ...
    geom_text(aes(label = label), size = txt_side, color = "black", fontface = "bold") +
    ...
    theme_void() +
    theme(
      axis.text.y  = element_text(size = txt_side * 1.2, hjust = 1,
                                  margin = margin(r = 1)),
...
build_hub_network <- function(mod, top_n = TOP_N, edge_quant = EDGE_QUANT) {
  ...
  if (length(periph_prots) > 0) {
    ...
    periph_df <- tibble(
      ...
      x = periph_x, y = periph_y, gene = periph_gene
    )
  }
  ...
  if (!is.null(hull_centroids) && nrow(hull_centroids) > 0) {
    p <- p +
      geom_label(data = hull_centroids,
                 ...
                 inherit.aes = FALSE,
                 size = txt_label * 0.8, fontface = "bold",
                 fill = scales::alpha("white", 0.85),
                 linewidth = 0.2, label.padding = unit(1, "mm"))
  }

  p <- p +
    geom_node_text(aes(label = label_show), size = txt_label * 0.7, repel = TRUE,
  ...
  p <- p +
    ...
    theme_void() +
    theme(
      plot.title    = element_text(face = "bold", size = txt_title),
      plot.subtitle = element_text(size = txt_sub, color = "grey30", face = "italic"),
...
panel_E <- panel_grid +
  plot_annotation(
    ...
    theme = theme(
      plot.title    = element_text(face = "bold", size = txt_title * 1.5),
      plot.subtitle = element_text(size = txt_sub * 1.2, color = "grey30",
                                   face = "italic")
    )
  )

# cor.test() CIs for hub kME and GS (stat audit)
hub_ci_list <- list()
...
ggsave(file.path(RPT, "panel_E_hub_network.pdf"), panel_E,
       width = PE_W, height = PE_H, units = "mm",
       device = pdf_device, limitsize = FALSE)
ggsave(file.path(RPT, "panel_E_hub_network.png"), panel_E,
       width = PE_W, height = PE_H, units = "mm",
