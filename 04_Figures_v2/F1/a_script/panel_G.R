# Figure 1 — Panel G: UpSet Plot (Dual Bar + Dot Matrix)
# Sources panel_F for sig_sets. Outputs: pG_bars, pG_dots (ggplot objects)
...
if (!exists("sig_sets")) source("04_Figures_v2/F1/a_script/panel_F.R")

RPT <- "04_Figures_v2/F1/b_reports"
DAT <- "04_Figures_v2/F1/c_data"
...
lbl_sz <- scale_text(BASE_COUNT, PG_W)

pG_bars <- ggplot(bar_long, aes(x, count, fill = direction)) +
  ...
  labs(y = "Intersection\nsize",
       ...
       subtitle = sprintf(
         "\u03A0 < 0.05 DEPs; %d \u03A0 / %d FDR total (%d mixed excl.)",
...
pG_dots <- ggplot() +
  ...
  theme(axis.text.x  = element_blank(), axis.ticks = element_blank(),
        ...
        axis.text.y  = element_text(size = FIG_AXIS_TEXT - 2, face = "bold",
                                     margin = margin(r = 1)),
...
write.csv(overlap_df, file.path(DAT, "audit_panel_G_overlap_enrichment.csv"), row.names = FALSE)

pG_combined <- (pG_bars / pG_dots) + plot_layout(heights = c(0.80, 0.20))

ggsave(file.path(RPT, "panel_G_upset.pdf"), pG_combined,
       width = PG_W, height = 120, units = "mm", device = pdf_device)
ggsave(file.path(RPT, "panel_G_upset.png"), pG_combined,
       width = PG_W, height = 120, units = "mm", dpi = 300)
