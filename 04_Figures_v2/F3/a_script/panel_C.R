...
melov_df <- read_csv("04_Figures_v2/F3/c_data/reversal_tests/melov_permutation.csv",
                      show_col_types = FALSE)
...
pC <- ggplot(mapping = aes(x = logFC_Aging, y = logFC_Training_Old)) +
  ...
  # NS proteins as standard scatter
  ...
  # Quadrant labels (inset from edges to avoid data overlap)
  ...
  annotate("label", x = xlim_range[1] * 0.95, y = ylim_range[1] * 0.95,
           ...
           color = "#DC2626", fill = alpha("white", 0.92),
           label.padding = unit(3, "pt")) +
  # Melov magnitude-reversal annotation
  annotate("label",
           x = xlim_range[2] * 0.45,
