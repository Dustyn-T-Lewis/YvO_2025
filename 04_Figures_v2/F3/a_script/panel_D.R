...
# === 3. QUADRANT PEAKS AND HOTSPOT GENES ======================================
...
pD_heat <- ggplot(hmat_df, aes(x = row, y = col, fill = neg_log10_pvalue)) +
  ...
  geom_vline(xintercept = mid_r + 0.5, linetype = "dashed",
             color = "white", linewidth = 0.4, alpha = 0.7) +
  # Quadrant names — x: Aging UP→DOWN (left=UP), y: Training DOWN→UP (bottom=DOWN)
...
# Piecewise y-axis compresses outlier p-values; slashes mark the break.
