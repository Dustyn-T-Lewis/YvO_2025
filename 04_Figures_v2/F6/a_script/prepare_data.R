...
# - Loads WGCNA module assignments + imputed data
# - Computes module eigengenes (MEs), signed kME
# - Splits eigengenes by Pre/Post, computes deltas
# - Computes gene significance (baseline expr vs delta phenotype)
# - Computes baseline prediction correlations + top-3 modules
# - Saves all shared objects for panel consumption

# ── STAT AUDIT (2026-02-27) ──────────────────────────────────────────────────
#   Gene significance (gs_vl, gs_lbm): Pearson r of baseline expression
#   vs delta phenotype — no CI computed here; downstream panels should add.
#   kME computed on full datExpr (all timepoints). This is standard WGCNA
...
DAT <- "04_Figures_v2/F6/c_data"
RPT <- "04_Figures_v2/F6/b_reports"
dir.create(DAT, recursive = TRUE, showWarnings = FALSE)
dir.create(RPT, recursive = TRUE, showWarnings = FALSE)

# ── 1. Load WGCNA module assignments ─────────────────────────────────────────
wgcna_dir <- if (dir.exists("04_Figures_v2/F5/c_data/wgcna")) {
  "04_Figures_v2/F5/c_data/wgcna"
...
# ── 2. Load imputed data + metadata ──────────────────────────────────────────
message("Loading imputed data...")
...
message(sprintf("  MEs: %d samples x %d eigengenes", nrow(MEs), ncol(MEs)))

# ── 4. Per-subject eigengene splits ──────────────────────────────────────────
...
# ── 6. Gene significance ────────────────────────────────────────────────────
message("Computing gene significance...")
...
# ── 7. Signed kME ───────────────────────────────────────────────────────────
message("Computing signed kME...")
kME_all <- signedKME(datExpr, MEs)

# ── 8. Baseline prediction correlations ──────────────────────────────────────
...
# Save top3, common_subj, mod_bio_labels, outcome_labels, delta vectors
saveRDS(list(
  top3           = top3,
