...
protocol (Dabke et al., 2021; Lazar et al., 2016). For each of 20
iterations:
...
## 6b: MNAR Post-Imputation Audit {#sec-mnar-audit}

Because BPCA is a MAR-oriented method, it is important to verify that
imputed values for MNAR-classified proteins are biologically plausible.
MNAR proteins are expected to have true intensities near or below the
detection limit; if BPCA imputes them at unrealistically high values,
it would inflate their apparent abundance and distort downstream
analyses.

We examine two diagnostics for the 331 MNAR proteins:

- **Panel A:** Pre vs. post-imputation mean scatter, colored by percent
  missingness. MNAR proteins should show modest negative shifts (imputed
  values below the observed mean), with larger shifts for proteins with
  more missingness.
- **Panel B:** Imputation shift vs. missingness. A downward trend
  confirms that higher-missingness MNAR proteins receive lower imputed
  values, consistent with the left-censoring mechanism.
...
(p_mnar_a | p_mnar_b) + plot_annotation(
  title = sprintf("MNAR Post-Imputation Audit (%d proteins, %s method)",
                  length(mnar_genes), best))
...
**Interpretation:** If the MNAR audit shows that BPCA imputes MNAR
proteins with negative mean shifts (post < pre), this is reassuring ---
it means the method is correctly recognizing that missing values for
these proteins correspond to lower-than-average intensities. Large
positive shifts for high-missingness proteins would be concerning and
might warrant switching to a hybrid method (e.g., BPCA for MAR + QRILC
for MNAR) despite its slightly higher overall NRMSE.
