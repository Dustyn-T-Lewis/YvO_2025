################################################################################
#   Figure 3 — Panel G: Per-Contrast ORA Bar Chart (4 Databases)
#
#   Runs over-representation analysis (hypergeometric test, BH correction)
#   on each F3 class gene set using 4 databases:
...
#
#   Gene lists (direction-collapsed, mutually exclusive from F3 classes):
#     Aging:    "Aging Up" + "Aging Down"
#     Reversal: "Rev. (Age Up)" + "Rev. (Age Down)"
#
#   Generates:
...
# ==============================================================================
# 1. BUILD GENE LISTS (direction-collapsed from F3 classification)
# ==============================================================================
...
gene_lists <- list(
  "Aging"    = class_df %>%
    filter(f3_class %in% c("Aging Up", "Aging Down")) %>% pull(gene),
  "Reversal" = class_df %>%
    filter(f3_class %in% c("Rev. (Age Up)", "Rev. (Age Down)")) %>% pull(gene)
)
...
if (nrow(ora_combined) > 0) {
  ...
  contrast_order <- c("Aging", "Reversal")
