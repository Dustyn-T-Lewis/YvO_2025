...
r_annotations <- tibble(
  ...
  label = sprintf("r = %.2f", c(r_young, r_old))
)
...
pB12 <- ggplot(scatter_df, aes(x = cv_pre, y = cv_post)) +
  ...
  geom_point(aes(color = max_cv_capped), alpha = 0.35, size = 0.6) +
  ...
  labs(title = "Per-Protein Variability (CV%)",
       subtitle = "Pre vs Post; labels = top 15 by max CV%",
  ...
  theme(plot.title    = element_text(hjust = 0, size = FIG_TITLE_SIZE, face = "bold"),
        ...
        legend.title = element_text(face = "bold", size = FIG_LEGEND_TITLE - 1),
...
pB3 <- ggplot(delta_wide, aes(x = dcv_Young, y = dcv_Old)) +
  ...
  geom_point(aes(color = mean_dcv_capped), alpha = 0.4, size = 0.6) +
