...
imp_summary_lines <- readLines("02_Imputation/c_data/09_imputation_summary.txt")
BEST_IMP_METHOD <- toupper(trimws(sub(".*=\\s*", "",
                                      grep("^best_method", imp_summary_lines, value = TRUE))))
...
pE <- ggplot(pca_df, aes(x = PC1, y = PC2, color = group, shape = group)) +
  ...
  labs(title = "Principal Component Analysis (PCA)",
       subtitle = sprintf("%s proteins | %s-imputed | n = %d",
       ...
       tag = "E") +
  FIG_THEME + theme(legend.position = c(0.85, 0.15),
