...
dxa_bar_data <- group_summary %>%
  select(Group, Timepoint, Group_Time, dxa_mean, dxa_sem)

# Y-axis range for bracket positioning
y_max_left <- max(dxa_bar_data$dxa_mean + dxa_bar_data$dxa_sem) * 1.02

pB_left <- ggplot(dxa_bar_data, aes(x = Group_Time, y = dxa_mean, fill = Group_Time)) +
  geom_col(width = 0.65, color = "grey30", linewidth = 0.3) +
  geom_errorbar(aes(ymin = dxa_mean - dxa_sem, ymax = dxa_mean + dxa_sem),
                width = 0.2, linewidth = 0.4) +
  geom_text(aes(label = sprintf("%.1f", dxa_mean)),
...
delta_dxa <- pheno_wide %>%
  group_by(Group) %>%
  summarise(
    mean_delta = mean(delta_DXA, na.rm = TRUE),
    sem_delta  = sd(delta_DXA, na.rm = TRUE) / sqrt(sum(!is.na(delta_DXA))),
    .groups    = "drop"
  )
...
y_max_right <- max(delta_dxa$mean_delta + delta_dxa$sem_delta)

pB_right <- ggplot(delta_dxa, aes(x = Group, y = mean_delta, fill = Group)) +
  ...
  labs(y = expression(Delta * " LBM (kg)"),
       x = NULL) +
  THEME_PUB +
