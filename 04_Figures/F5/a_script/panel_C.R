################################################################################
#   YvO Figure 5 — Panel C: Per-Module Triptych
...
DB_COLORS   <- c("GO:BP" = "#00796B", "GO:CC" = "#26A69A", "GO:MF" = "#CD5C5C")
Z_CAP <- 2
X_CAP <- 12   # tighter bar axis cap for readability

GROUP_COLS <- c("Young_Pre", "Young_Post", "Old_Pre", "Old_Post")
GROUP_LABS <- c("Y-Pre", "Y-Post", "O-Pre", "O-Post")

# Key trait per module (for eigengene strip)
MODULE_TRAIT <- c(
  blue      = "age",
  brown     = "training",
  turquoise = "age",
...
# Human-readable module biology labels (updated from GO enrichment)
MODULE_BIOLOGY <- c(
  blue      = "Derived from GO enrichment",
  brown     = "Derived from GO enrichment",
  turquoise = "Derived from GO enrichment",
  green     = "Derived from GO enrichment",
  black     = "Derived from GO enrichment",
  pink      = "Derived from GO enrichment"
)
...
cat(sprintf("  Group sizes: Y-Pre=%d, Y-Post=%d, O-Pre=%d, O-Post=%d\n",
...
# === STEP 2: Per-module data preparation ======================================
...
for (mod in KEY_MODULES) {
  ...
  cat(sprintf("  Module %s: %d proteins\n", mod, n_mod))

  # --- Z-scored group means (heatmap data) ---
  ...
  cat(sprintf("    Top enrichment terms: %d\n", nrow(mod_enrich)))

  # --- 1:1 greedy protein -> pathway assignment ---
  ...
  n_mapped <- sum(sankey_list[[mod]]$pathway != "Unmapped")
  cat(sprintf("    Mapped: %d / %d\n", n_mapped, n_mod))
}

# === STEP 3: Eigengene data ===================================================
...
# === STEP 4: Shared x-axis max for enrichment bars ============================
...
cat(sprintf("  Shared enrichment bar x-max: %.1f\n", shared_x_max))

# === STEP 5: build_triptych_row() function ====================================
...
build_triptych_row <- function(mod, show_xlab, shared_x_max) {
  ...
  # ===== Accent bar (colored strip with module name) ==========================
  ...
  p_ht <- ggplot(ht_long, aes(x = group, y = uniprot_id, fill = z)) +
    ...
    theme(
      ...
      axis.text.x  = if (show_xlab) element_text(size = 8, angle = 45, hjust = 1,
                                                   face = "bold")
                      else element_blank(),
  ...
  if (n_pw == 0) {
    ...
    Y_SPAN <- n_genes
    X_GENE <- 0.0    # gene bars flush at left edge (no gap to heatmap)
    X_PW   <- 1.5
    BAR_W  <- 0.06
    ...
    ribbon_polys <- pmap_dfr(ribbon_input, function(...) {
      ...
      make_sigmoid_ribbon(
        x0 = X_GENE + BAR_W / 2,
        x1 = X_PW   - BAR_W / 2,
        y0_top = args$y_top_gene,
        y0_bot = args$y_bot_gene,
        y1_top = args$slot_top,
        y1_bot = args$slot_bot,
    ...
    # Pathway label size: scale with row count
    pw_label_size <- if (n_pw <= 4) 3.5 else if (n_pw <= 7) 3.0 else 2.6
  ...
  # ===== C3: PEA Enrichment Bars =============================================
  # Only show terms that have at least one gene in the sankey
  ...
  if (nrow(cl_enrich_vis) == 0) {
    ...
    # Join enrichment stats with sankey y-positions so bars align vertically
    ...
    # Bar label size: scale with term count
    bar_label_size <- if (nrow(bar_data) <= 4) 3.2 else if (nrow(bar_data) <= 7) 2.8 else 2.4
    ...
    p_bars <- ggplot(bar_data) +
      ...
      theme(
        axis.text.y  = element_blank(),
        axis.ticks.y = element_blank(),
        axis.text.x  = if (show_xlab) element_text(size = 8, face = "bold") else element_blank(),
        axis.ticks.x = if (show_xlab) element_line() else element_blank(),
        axis.title.x = if (show_xlab) element_text(size = 9, face = "bold") else element_blank(),
        panel.border = element_rect(colour = "grey70", linewidth = 0.3, fill = NA),
        plot.margin  = margin(t = 1, r = 1, b = if (show_xlab) 4 else 0, l = 0)
      )
  }

  # ===== C3b: Trait Correlation Tile Strip ====================================

  trait_mod <- trait_heatmap_df %>%
    filter(module == paste0("ME", mod), trait %in% KEY_TRAITS) %>%
    mutate(trait_label = factor(trait_label,
                                levels = rev(KEY_TRAIT_LABELS)))

  p_trait <- ggplot(trait_mod, aes(x = 1, y = trait_label, fill = cor)) +
    geom_tile(color = "grey60", linewidth = 0.3) +
    geom_text(aes(label = stars), size = 3, fontface = "bold", vjust = 0.75) +
    ...
    theme(
      ...
      axis.title.x   = if (show_xlab) element_text(size = 8, face = "bold.italic")
                        else element_blank(),
      panel.border   = element_rect(colour = "grey70", linewidth = 0.3, fill = NA),
      plot.margin    = margin(t = 1, r = 1, b = if (show_xlab) 4 else 0, l = 1)
    )

  # ===== C4: Paired Slope Plot (Young | Old, Pre->Post) =======================
  ...
  p_eigen <- ggplot(eig_slope, aes(x = time_fct, y = eigengene)) +
    ...
    THEME_PUB +
    theme(
      plot.title       = element_text(size = 8.5, face = "bold.italic", color = "grey25",
                                       hjust = 0.5, margin = margin(0, 0, 2, 0)),
      strip.text       = element_text(size = 8, face = "bold", color = "grey15"),
      strip.background = element_rect(fill = "grey93", colour = NA),
      axis.text.x  = if (show_xlab) element_text(size = 8, face = "bold")
                      else element_blank(),
      axis.ticks.x = if (show_xlab) element_line() else element_blank(),
      axis.text.y  = element_text(size = 7, face = "bold"),
      axis.title.y = if (show_xlab) element_text(size = 9, face = "bold") else element_blank(),
  ...
  # ===== Assemble row ========================================================
  # Layout: heatmap(8%) | sankey(30%) | bars(26%) | trait r(8%) | eigengene(18%)

  row <- (p_ht | p_sankey | p_bars | p_trait | p_eigen) +
    plot_layout(widths = c(0.08, 0.30, 0.26, 0.08, 0.18),
                nrow = 1)
...
# === STEP 7: Compressed row heights ==========================================
# Use sqrt scaling to compress large modules, elevate small ones

raw_sizes <- module_sizes[KEY_MODULES]
row_heights <- sqrt(raw_sizes) / sum(sqrt(raw_sizes))
row_heights <- pmax(row_heights, 0.10)  # 10% floor for readability
row_heights <- row_heights / sum(row_heights)

cat(sprintf("  Row heights: %s\n",
            paste(sprintf("%s=%.3f", KEY_MODULES, row_heights), collapse = ", ")))

# === STEP 8: Assemble panel ===================================================

panel_C <- wrap_plots(panels, ncol = 1, heights = row_heights) +
  plot_annotation(
    title    = "C   Module Functional Characterization",
    subtitle = "Z-scored group means  |  Gene-pathway Sankey  |  GO enrichment  |  Trait r  |  Eigengene slopes",
    theme = theme(
      plot.title    = element_text(face = "bold", size = 14, color = "black"),
      plot.subtitle = element_text(size = 9, color = "grey25", face = "bold.italic")
    )
  )

# === STEP 9: Save =============================================================

ggsave(file.path(RPT_DIR, "panel_C_triptych.pdf"), panel_C,
       width = 580, height = 340, units = "mm",
       device = pdf, limitsize = FALSE)
ggsave(file.path(RPT_DIR, "panel_C_triptych.png"), panel_C,
       width = 580, height = 340, units = "mm",
       dpi = 300, limitsize = FALSE)

cat("  Panel C figures saved\n")
...
# ============================================================================
# PHASE 4a: MODULE EIGENGENE REVERSAL (Paired t-test)
# For each age-associated module, test whether eigengenes shift toward
# Young levels after training (Old Post vs Old Pre, paired).
# Reference: Langfelder & Horvath 2008; Lavin et al. 2020 (PMID 32625117)
# ============================================================================

cat("\n--- Module Eigengene Reversal Tests ---\n")

# Identify age-associated modules from trait correlation data
# (trait_cor / pval_bh loaded in F5 setup)
# Format: module (rows=ME*) x traits (columns=age_num, time_num, ...)

# Find modules with significant age correlation (BH p < 0.05)
age_modules <- pval_bh %>%
...
# Get age correlations for annotation
age_cors <- trait_cor %>%
  dplyr::select(module, cor = age_num) %>%
  mutate(module_color = gsub("^ME", "", module))

cat(sprintf("  Age-associated modules (BH p < 0.05): %s\n",
            paste(age_modules, collapse = ", ")))

# Split eigengenes by timepoint (using meta from F5 setup)
...
for (mod in age_modules) {
  ...
  # Old Pre and Post eigengenes (paired by subject)
  ...
  # Reversal as percentage of the age gap
...
if (length(reversal_results) > 0) {
  ...
  # METHODS NOTE: "Modules were defined on the full dataset (Pre + Post).
  # Module preservation analysis (Panel F, all Zsummary > 10) confirms
  # co-expression structure is stable across timepoints, mitigating
  # circularity concerns."
} else {
  cat("  No age-associated modules found for reversal test\n")
}

# ============================================================================
# PHASE 4b: AGE GAP CLOSURE METRIC
# For each module, compare |Young_Pre_mean - Old_Pre_mean| (gap_pre) vs
# |Young_Post_mean - Old_Post_mean| (gap_post). If gap_post < gap_pre,
# training narrows the age gap.
# ============================================================================

cat("\n--- Age Gap Closure ---\n")

# Use all non-grey modules
