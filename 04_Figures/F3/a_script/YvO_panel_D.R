...
# Quadrant max values
# Top-left: Aging Up ∩ Training Up = Exacerbated Up
# Top-right: Aging Up ∩ Training Down = Reversed
# Bottom-left: Aging Down ∩ Training Up = Reversed
# Bottom-right: Aging Down ∩ Training Down = Exacerbated Down
...
n_UU <- length(intersect(mid_aging, mid_old))
bot_aging <- rank_aging[(indices[mid_r]+1):n_shared]
bot_old   <- rank_old[(indices[mid_c]+1):n_shared]
n_DD <- length(intersect(bot_aging, bot_old))
n_UD <- length(intersect(mid_aging, bot_old))
n_DU <- length(intersect(bot_aging, mid_old))
