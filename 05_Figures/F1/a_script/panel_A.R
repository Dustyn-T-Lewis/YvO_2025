...
#
#   Requires from setup: norm_df, meta, samp_names, GROUP_FILL, FIG_THEME,
#                         RPT_DIR, KEY_TEXT
...
# Wilcoxon brackets — BH correction across 4 comparisons
...
# Significance brackets with BH-adjusted p-values and Cliff's delta
.signif_label <- function(p, d) {
  star <- if (p < 0.001) "***" else if (p < 0.01) "**" else if (p < 0.05) "*" else "ns"
  sprintf("%s  |d| = %.2f", star, abs(d))
}
