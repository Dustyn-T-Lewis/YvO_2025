...
#
#   Requires from setup: dep_df, CONTRASTS, CONTRAST_COLORS, FIG_THEME,
#                         RPT_DIR, DAT_DIR
...
# Clopper-Pearson 95% CI on pi < 0.05 fraction per contrast
...
frac_list <- lapply(CONTRASTS, function(ctr) {
  ...
  tibble(
    ...
    threshold = c("p < 0.05", "q < 0.05", expression(Pi) < 0.05),
...
# Use character Pi symbol for threshold labels
frac_list <- lapply(CONTRASTS, function(ctr) {
  fdr_col <- paste0("adj.P.Val_", ctr)
  p_col   <- paste0("P.Value_", ctr)
  tibble(
