################################################################################
#   Figure 3 — Panel B: Reversal Volcano Ring (wide)
#   Generates: panel_B_volcano.pdf/png, c_data/panel_B/volcano_reversal.csv
################################################################################

if (!exists("dep_df")) source("05_Figures/F3/a_script/YvO_F3_setup.R")
source("05_Figures/shared/volcano_ring.R")

message("Panel B: Reversal volcano ring...")

# ── Ring term helpers (shared with panel_A; re-defined for standalone use) ────
...
ring_rev      <- build_ring_with_gaps(top_terms_rev, "Reversal", fgsea_all, n_each = 5)

pB <- make_volcano_ring(
  de_df = dep_df, go_df = fgsea_all, contrast = "Reversal",
...
dep_df %>%
  ...
  arrange(pi_score) %>%
  write_csv(file.path(DAT_DIR, "panel_B", "volcano_reversal.csv"))
