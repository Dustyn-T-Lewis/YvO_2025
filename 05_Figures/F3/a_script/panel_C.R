...
# Reversal %: proteins with opposite signs, |logFC| > 0.2 gate, BCa bootstrap CI
...
q_df <- scatter_df %>%
  mutate(q = case_when(
    ...
    logFC_Aging > 0 & logFC_Training_Old > 0 ~ "TR",   # top-right: exacerbated
    TRUE ~ "BL"))                                        # bottom-left: exacerbated
...
# Melov annotation values from setup
...
pC <- ggplot(mapping = aes(x = logFC_Aging, y = logFC_Training_Old)) +
  ...
  annotate("label",
           ...
           label = sprintf("Magnitude reversal: %.1f%%, p = %.2f\n(Melov permutation, %d aging-sig. proteins)",
                           melov_rev_pct, melov_p, melov_n),
           hjust = 0.5, vjust = 1, size = 2.8, fontface = "italic",
...
# Reversal scatter statistics CSV
tibble(
  ...
  n        = c(n_obs, n_obs, nrow(reversal_set), melov_n),
  ...
  write_csv(file.path(DAT_DIR, "panel_C", "reversal_scatter_stats.csv"))
