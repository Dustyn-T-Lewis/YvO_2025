...
Because the HPA filter removes all proteins lacking skeletal muscle
annotation, it could inadvertently discard aging-relevant proteins that
are genuinely expressed in muscle but absent from the HPA skeletal
muscle reference (e.g., immune infiltration markers, ECM remodeling
collagens, satellite cell markers). We screen the 165 removed proteins
against curated gene-name patterns associated with skeletal muscle
aging, atrophy, and inflammation (Distefano & Bhatt, 2020; Murton
et al., 2015).

```{r hpa-sensitivity-audit}
#| label: hpa-sensitivity-audit
...
if (n_flagged > 0) {
  hpa_audit %>% filter(aging_relevant) %>% kable(caption = "Aging-relevant genes removed by HPA filter")
}

write_csv(hpa_audit, file.path(data_dir, "02c_hpa_sensitivity_audit.csv"))
```

If any aging-relevant genes are flagged, they should be examined
individually to determine whether their removal is justified (e.g.,
the protein is genuinely non-muscle) or whether the HPA reference is
incomplete for that gene. A small number of flagged genes (0--3) is
expected and acceptable; a large number would warrant revisiting the
HPA filter strategy.
...
group_prop <- dal$metadata %>%
  ...
  bind_cols()
group_prop <- as.data.frame(group_prop)
rownames(group_prop) <- rownames(dal$data)
...
# Per-protein missingness for downstream reference (includes group detection proportions)
miss_by_protein <- annot_df %>%
  filter(uniprot_id %in% removed_uniprots) %>%
  dplyr::select(uniprot_id, gene, description) %>%
  bind_cols(
    as_tibble(group_prop[removed_uniprots, , drop = FALSE])
  )
