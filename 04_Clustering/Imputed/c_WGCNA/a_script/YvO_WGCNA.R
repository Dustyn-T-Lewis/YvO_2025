...
REPORT_DIR <- file.path(base_dir, "04_Clustering", "Imputed", "c_WGCNA", "b_reports")
DATA_DIR   <- file.path(base_dir, "04_Clustering", "Imputed", "c_WGCNA", "c_data")
...
plotDendroAndColors(net$dendrograms[[1]], module_colors[net$blockGenes[[1]]],
                    "Module colors",
                    main = "Protein dendrogram and module colors (Imputed)",
...
labeledHeatmap(
  ...
  cex.text = 0.6,
  zlim = c(-1, 1),
  main = "Module-trait correlations (Imputed)"
)
dev.off()
...
p_me <- ggplot(ME_df, aes(x = group, y = eigengene, fill = group)) +
  ...
  labs(x = NULL, y = "Module Eigengene", title = "Module eigengenes by group (Imputed)") +
